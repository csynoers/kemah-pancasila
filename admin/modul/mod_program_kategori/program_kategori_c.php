...
{
    // Insert
    if ($module=='program_kategori' AND $act=='insert')
    {
        ...
        $database->insert($table="program_kategori", $array=$form_data);
...
{
    // Update
    if ($module=='program_kategori' AND $act=='update')
    {
        ...
        $database->update($table="program_kategori", $array=$form_data, $fields_key="id_program_kategori", $id="$_POST[id]");
...
{
    // Delete
    if ($module=='program_kategori' AND $act=='delete')
    {
        //validasi 
        $show   = $database->select($fields="id_program", $table="program", $where_clause="WHERE id_program_kategori = '$_GET[id]'", $fetch='');
        if($show['id_program'] != '')
        {
            echo "<script>alert('Maaf! Kategori gagal dihapus karena masih digunakan dalam beberapa program.'); window.location = '../../media.php?module=$module';</script>";            
        }
        else
        {
            $database->delete($table="program_kategori", $fields_key="id_program_kategori", $id="$_GET[id]");
        }
