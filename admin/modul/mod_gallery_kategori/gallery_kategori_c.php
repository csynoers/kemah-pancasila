...
{
    // Insert
    if ($module=='gallery_kategori' AND $act=='insert')
    {
        ...
        $database->insert($table="gallery_kategori", $array=$form_data);
...
{
    // Update
    if ($module=='gallery_kategori' AND $act=='update')
    {
        ...
        $database->update($table="gallery_kategori", $array=$form_data, $fields_key="id_kategori", $id="$_POST[id]");
...
{
    // Delete
    if ($module=='gallery_kategori' AND $act=='delete')
    {
        //validasi 
        $show   = $database->count_rows($table="gallery", $where_clause="WHERE id_kategori = '$_GET[id]'");
        if($show>0)
        {
            echo "<script>alert('Maaf! Kategori gagal dihapus karena masih digunakan dalam beberapa gallery.'); window.location = '../../media.php?module=$module';</script>";            
        }
        else
        {
            $database->delete($table="gallery_kategori", $fields_key="id_kategori", $id="$_GET[id]");
        }
